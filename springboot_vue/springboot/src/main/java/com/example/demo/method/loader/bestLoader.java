...
//            Class.forName("org.postgresql.Driver");
        ...
        try {
            ...
            stmtAddStock = con5.prepareStatement("insert into inventory(id,product_model,supply_center,quantity)"
                    + "values(?,?,?,?)");

            stmtUpdateStock = con5.prepareStatement("update inventory set quantity=? where product_model=?"
                    + " and supply_center=?");
...
//                        System.out.println(parts[0]);
                            ...
                            if (stockMap.get(unique) != null) {
                                ...
                                stmtUpdateStock.setString(2, stockIn.model);
                                stmtUpdateStock.setString(3, stockIn.center);
                ...
                while ((line = infile.readLine()) != null) {
                    ...
                    if (parts.length > 1) {
                        ...
                        if (!staffMap.get(parts[8]).type.equals("Salesman") || stockMap.get(unique) == null
                            ...
                            stmtUpdateStock.setString(2, parts[2]);
                            stmtUpdateStock.setString(3, stockMap.get(unique).center);
                            stmtUpdateStock.addBatch();
                            if (i % BATCH_SIZE == 0) {
            ...
            try (BufferedReader infile
                ...
                while ((line = infile.readLine()) != null) {
                    ...
                    if (orderMap.get(order_unique) == null || stockMap.get(stock_unique).quantity -
                        ...
                        stmtUpdateStock.setString(2, parts[1]);
                        stmtUpdateStock.setString(3, stockMap.get(stock_unique).center);
                        stmtUpdateStock.addBatch();
                        if (i % BATCH_SIZE == 0) {
                    ...
                    if (parts.length > 1) {
                        ...
                        if (deleteMap.get(de) == null) {
                            ...
                            if (id != null) {
                                ...
                                stmtUpdateStock.setInt(1, stockMap.get(stock_unique).quantity);
                                stmtUpdateStock.setString(2, stockMap.get(stock_unique).model);
                                stmtUpdateStock.setString(3, stockMap.get(stock_unique).center);
                                stmtUpdateStock.addBatch();
    ...
    static class stockIns {
        ...
        public stockIns(int id, String center, String model, String supply_staff, int quantity) {
            this.id = id;
            this.center = center;
            this.model = model;
    ...
    public static String WuSuoWei(String s) throws SQLException {
        ...
        if (strings[0].equals("insert")) {
            ...
            String sql2 = "select * from " + strings[1] + " where " + strings[2] + ";";
            ...
            stmtDe.execute("delete from " + strings[1] + " where " + strings[2] + ";");
        } else if (strings[0].equals("update")) {
            Connection con = dataSource.getConnection();
            Statement stmtUp = con.createStatement();
            stmtUp.execute("update " + strings[1] + " set " + strings[2] + " where " + strings[3] + ";");
        } else {
            return "false";
        }
        return "true";
    ...
    public  void loadData() throws SQLException {
        ...
        if (flag) {
            ...
            if (con5 != null) {
                ...
                stmt0.execute("truncate table center,enterprise,model,staff,contract,orders,inventory");
                con5.commit();
                stmt0.close();
            }
